...
With the introduction of the frame_metadata V15 and the [RFC-78](https://polkadot-fellows.github.io/RFCs/approved/0078-merkleized-metadata.html)
...
You will notice that the example above is very similar to the one in [Make a balance transfer](./guides/make-transfer). The difference here are the following:
...
Now, notice that we are providing the `metadataHash` in the `SigningPayload` and `ExtrinsicPayload` objects.
