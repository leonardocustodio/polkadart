...
import 'package:polkadart_scale_codec/polkadart_scale_codec.dart' as scale_codec show Input;
import 'package:code_builder/code_builder.dart' show Class, Code, declareFinal, literalList, Block, refer, Reference, Method, Field, FieldModifier, Parameter, Constructor, Enum, EnumValue, literalNum, TypeDef, TypeReference, MethodModifier;
...
Class createCompositeClass(
  ...
  List<generator.Field> fields,
  {
    ...
    final classType = TypeReference((b) => b
      ..symbol = typeName
    );
...
Class createCompositeCodec(String codecName, Reference typeName, List<generator.Field> fields) {
  return Class((classBuilder) {
    classBuilder
      ...
      ..methods.add(Method.returnsVoid((b) => b
        ...
        ..body = Block.of(fields.map((field) => field.codec.encode(refer('obj').property(field.name)).statement))
      ))
      ..methods.add(Method((b) => b
        ..name = 'decode'
        ..returns = typeName
        ...
        ..body = Block((b) => b
          ..statements.add(typeName.newInstance([], {
      ...
      ..methods.add(Method((b) => b
        ...
        ..body = Block((b) => b
          ...
          ..statements.addAll(fields.map((field) => refer('size').assign(
            refer('size').operatorAdd(
              field.codec.codecInstance().property('sizeHint').call([refer('obj').property(field.name)])
            )
          ).statement))
  ...
  List<v.Variant> variants,
    ...
    classBuilder
      ...
      ..methods.add(Method((b) => b
        ...
        ..returns = constants.uint8List
        ..body = Block.of([
          Code.scope((a) => 'final output = ${a(constants.byteOutput)}(codec.sizeHint(this));'),
      ...
      ..methods.addAll(variants.map((variant) => Method((b) => b
        ...
        ..body = variant.fields.isEmpty
          ...
          : Block.of([
              ...
              Block.of(variant.fields.map((field) => Code('${field.name}: ${field.name},'))),
              Code(');'),
            ])
        ..optionalParameters.addAll(variant.fields.map((field) => Parameter((b) => b
          ..required = field.codec.primitive().isNullable != true
          ..named = true
          ..type = field.codec.primitive()
          ..name = field.name
  ...
  List<v.Variant> variants,
    ...
    classBuilder
      ...
      ..methods.add(Method((b) => b
        ...
        ..body = Block.of([
          declareFinal('index').assign(PrimitiveGenerator.u8.decode()).statement,
          Code('switch (index) {'),
            ...
            Code('default: throw Exception(\'$typeName: Invalid variant index: "\$index"\');'),
            Code('}'),
        ])
      ))
      ...
      ..methods.add(Method.returnsVoid((b) => b
        ...
        ..body = Block((b) => b
          ..statements.addAll([
            ...
            Code('default: throw Exception(\'$typeName: Unsupported "\$value" of type "\${value.runtimeType}"\');'),
            Code('}'),
          ])
        )
      ))
  ...
  v.Variant variant,
        ...
        ..methods.add(
          Method((b) => b
            ...
            ..body = Block((b) => b
              ...
              ..statements.addAll(variant.fields.map((field) => refer('size').assign(
                refer('size').operatorAdd(
                  field.codec.codecInstance().property('sizeHint').call([refer(field.name)])
                )
              ).statement))
              ..statements.add(refer('size').returned.statement)
            )
    ...
    classBuilder.methods.add(
      Method.returnsVoid((b) => b
        ...
        ..body = Block((b) => b
          ..statements.add(PrimitiveGenerator.u8.encode(literalNum(variant.index)).statement)
          ..statements.addAll(variant.fields.map((field) => field.codec.encode(refer(field.name)).statement)),
        )
      )
    );
  });
    ...
    enumBuilder
      ...
      ..fields.addAll([
        ...
        Field((b) => b
          ...
          ..static = true
          ..modifier = FieldModifier.constant
          ..type = refer('Codec<$typeName>', 'package:scale_codec/scale_codec.dart')
          ..assignment = Code('$codecName()')
        ),
      ])
...
Class createSimpleVariantCodec(
  ...
  List<v.Variant> variants,
    ...
    classBuilder
      ...
      ..mixins.add(refer('Codec<$typeName>', 'package:scale_codec/scale_codec.dart'))
      ...
      ..methods.add(Method((b) => b
        ...
        ..body = Block((b) => b
          ..statements.add(declareFinal('index').assign(PrimitiveGenerator.u8.decode()).statement)
          ..statements.add(Code('switch (index) {'))
          ..statements.add(Block.of(variants.map((variant) => Block.of([
            Code('case ${variant.index}:'),
            Code('return $typeName.${generator.Field.toFieldName(variant.name)};')
          ]))))
          ..statements.add(Code('default: throw Exception(\'$typeName: Invalid variant index: "\$index"\');'))
          ..statements.add(Code('}'))
        )
      ))
      ...
      ..methods.add(Method.returnsVoid((b) => b
        ...
        ..body = PrimitiveGenerator.u8.encode(refer('value').property('codecIndex')).statement
      ));
  });

TypeDef createTypeDef({required String name, required Reference reference}) => TypeDef((b) => b
  ..name = name
...
Class createTupleClass(
  int size,
    ...
    classBuilder
      ...
      ..constructors.add(Constructor((b) => b
        ..constant = true
        ..requiredParameters.addAll(List.generate(size, (index) => Parameter((b) => b
          ..toThis = true
          ..required = false
          ..named = false
          ..name = 'value$index'
      ...
      ..fields.addAll(List.generate(size, (index) => Field((b) => b
        ..name = 'value$index'
        ..type = refer('T$index')
        ..modifier = FieldModifier.final$
      )));
  ...
  int size,
    ...
    classBuilder
      ...
      ..methods.add(Method.returnsVoid((b) => b
        ...
        ..body = Block((b) => b
          ..statements.addAll(List.generate(size, (index) => Code('codec$index.encodeTo(tuple.value$index, output);')))
        )
      ))
      ..methods.add(Method((b) => b
        ..name = 'decode'
        ...
        ..body = Block((b) => b
          ...
          ..statements.addAll(List.generate(size, (index) => Code('codec$index.decode(input),')))
          ..statements.add(Code(');'))
        )
      ))
      ..methods.add(Method((b) => b
        ...
        ..body = Block((b) => b
          ..statements.add(Code('int size = 0;'))
          ..statements.addAll(List.generate(size, (index) => Code('size += codec$index.sizeHint(tuple.value$index);')))
  ...
  pallet.PalletGenerator generator,
    ...
    classBuilder
      ...
      ..methods.addAll(generator.storages.map((storage) => Method((builder) {
        ...
        ..returns = constants.future(storage.valueCodec.primitive(), nullable: storage.isNullable)
        ...
        ..body = Block((b) => b
          ...
          ..statements.add(
            declareFinal('hashedKey')
            .assign(
              refer('_$storageName')
              .property(
                storage.hashers.isEmpty
                ? 'hashedKey'
                : 'hashedKeyFor'
              ).call(
          ...
          ..statements.add(
            ...
            .assign(
              ...
              .call([literalList([refer('hashedKey')])])
              .property('then')
              .call([
                Method.returnsVoid((b) => b
                  ..requiredParameters.add(Parameter((b) => b
          ...
          ..statements.add(Code('  return _$storageName.decodeValue(value);'))
          ..statements.add(Code('}'))
          ..statements.add(storage.isNullable
            ? Code('return null; /* Nullable */')
            : storage.valueCodec.valueFrom(scale_codec.Input.fromBytes(storage.defaultValue)).returned.statement
          )
  ...
  PolkadartGenerator generator,
    ...
    classBuilder
      ...
      ..constructors.add(Constructor((b) => b
        ...
        ..initializers.addAll(generator.pallets.map((pallet) => Code.scope((a) => '${ReCase(pallet.name).camelCase} = ${a(pallet.queries())}(provider)')))
      ))
