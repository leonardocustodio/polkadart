...
void main() {
...

  {
    ...
    group('Encode Option<bool>:', () {
      test('\'true\' should encode to \'0x0101\'', () {
        final encoded =
            codec.encodeToHex(registry.getIndex('Option<bool>'), true);
        expect('0x0101', equals(encoded));
      });
      test('\'false\' should encode to \'0x0100\'', () {
        final encoded =
            codec.encodeToHex(registry.getIndex('Option<bool>'), false);
        expect('0x0100', equals(encoded));
      });
      test('\'null\' should encode to \'0x00\'', () {
        final encoded =
            codec.encodeToHex(registry.getIndex('Option<bool>'), null);
...

  {
    ...
    group('Decode Option<bool>:', () {
      test('\'0x0101\' should decode to \'true\'', () {
        final decoded =
            codec.decodeBinary(registry.getIndex('Option<bool>'), '0x0101');
        expect(true, equals(decoded));
      });
      test('\'0x0100\' should decode to \'false\'', () {
        final decoded =
            codec.decodeBinary(registry.getIndex('Option<bool>'), '0x0100');
        expect(false, equals(decoded));
      });
      test('\'0x00\' should decode to \'null\'', () {
        final decoded =
            codec.decodeBinary(registry.getIndex('Option<bool>'), '0x00');
    ...
    group('Encode Option<u128>:', () {
      test('\'1\' should encode to \'0x0101000000000000000000000000000000\'',
        ...
        final encoded = codec.encodeToHex(
            registry.getIndex('Option<u128>'), BigInt.from(1));
        expect('0x0101000000000000000000000000000000', equals(encoded));
      });
      test('\'null\' should encode to \'0x00\'', () {
        final encoded =
            codec.encodeToHex(registry.getIndex('Option<u128>'), null);
        expect('0x00', equals(encoded));
      });
    });
  }

  {
    ...
    group('Decode Option<u128>:', () {
      test('\'0x0101000000000000000000000000000000\' should decode to \'1\'',
          () {
        final decoded = codec.decodeBinary(registry.getIndex('Option<u128>'),
            '0x0101000000000000000000000000000000');
        expect(BigInt.from(1), equals(decoded));
      });
      test('\'0x00\' should decode to \'null\'', () {
        final decoded =
            codec.decodeBinary(registry.getIndex('Option<u128>'), '0x00');
        expect(null, equals(decoded));
      });
    });
  }

  {
    ...
    group('Encode Option<Compact<u8>>:', () {
      test('\'63\' should encode to \'0x01fc\'', () {
        final encoded =
            codec.encodeToHex(registry.getIndex('Option<Compact<u8>>'), 63);
        expect('0x01fc', equals(encoded));
      });
      test('\'null\' should encode to \'0x00\'', () {
        final encoded =
            codec.encodeToHex(registry.getIndex('Option<Compact<u8>>'), null);
        expect('0x00', equals(encoded));
      });
    });
  }
    ...
    group('Decode Option<Compact<u8>>:', () {
      test('\'0x01fc\' should decode to \'63\'', () {
        final decoded = codec.decodeBinary(
            registry.getIndex('Option<Compact<u8>>'), '0x01fc');
        expect(63, equals(decoded));
      });
      test('\'0x00\' should decode to \'null\'', () {
        final decoded = codec.decodeBinary(
            registry.getIndex('Option<Compact<u8>>'), '0x00');
        expect(null, equals(decoded));
      });
    });
  }
  ...
  group('Encode Option<BitVec<u8>>:', () {
    test('Lowest acceptable value is [0] and the output should be \'0x012000\'',
        () {
      final encoded =
          codec.encodeToHex(registry.getIndex('Option<BitVec<u8>>'), [0]);
    ...
    test(
        ...
        () {
      final encoded =
          codec.encodeToHex(registry.getIndex('Option<BitVec<u8>>'), [255]);
      expect(encoded, equals('0x0120ff'));
    });

    test('\'null\' should encode to \'0x00\'', () {
      final encoded =
          codec.encodeToHex(registry.getIndex('Option<BitVec<u8>>'), null);
      expect('0x00', equals(encoded));
    });
  });
  ...
  group('Decode Option<BitVec<u8>>:', () {
    test('On decoding \'0x012000\' we must get lowest acceptable value: [0]',
        () {
      final decoded = codec.decodeBinary(
          registry.getIndex('Option<BitVec<u8>>'), '0x012000');
    ...
    test('On decoding \'0x0120ff\' we must get lowest acceptable value: [255]',
        () {
      final decoded = codec.decodeBinary(
          registry.getIndex('Option<BitVec<u8>>'), '0x0120ff');
      expect(decoded, equals([255]));
    });

    test('\'0x00\' should decode to \'null\'', () {
      final decoded =
          codec.decodeBinary(registry.getIndex('Option<BitVec<u8>>'), '0x00');
      expect(null, equals(decoded));
    });
  });
