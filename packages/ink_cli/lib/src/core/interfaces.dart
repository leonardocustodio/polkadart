...
class Interfaces {
  ...
  String _makeStruct(CompositeCodecInterface type, int ti) {
    ...
    // Queue a callback that writes out the struct definition
    /* sink.push((final AbiOutput out) {
      out.line();
      ...
      out.block(
        start: 'class $name',
        cb: () {
          // Print each field as `final <type> <name>;`
          for (final field in type.fields) {
            final fieldName = field.name ?? '_unnamed';
            final fieldDocs = field.docs;
            out.blockComment(fieldDocs);
            final fieldTypeExp = use(field.type);
            final isOptional = _isOptional(field.type) ? '?' : '';
            // e.g. `final <fieldTypeExp><isOptional> fieldName;`
            out.line('final $fieldTypeExp$isOptional $fieldName;');
          }
          ...
          out.block(
            ...
            cb: () {
              for (final field in type.fields) {
                final fieldName = field.name ?? '_unnamed';
                /* final fieldTypeExp = use(field.type); */
                if (_isOptional(field.type)) {
                  out.line('this.$fieldName,');
                } else {
                  out.line('required this.$fieldName,');
                }
              }
            },
            end: '});',
  ...
  String _makeVariant(VariantCodecInterface variant, int ti) {
    ...
    /* sink.push((final AbiOutput out) {
      ...
      if (variant.variants.isEmpty) {
        // If no variants exist, it's effectively `Never`.
        out.line('typedef $name = Never;');
        return;
      }

      // For simplicity, we'll show how to do a sealed-like pattern with factory constructors:

      // We print a comment about the type
      out.line('abstract class $name {');
      out.line('  const $name();');

      // For each variant, we create a factory constructor or a separate class
      for (final v in variant.variants) {
        final variantClassName = '${name}_${v.name}';
        out.line();
        out.blockComment(v.docs);
        out.block(
          start: 'factory $name.${v.name}({',
          cb: () {
            if (v.fields.isNotEmpty) {
              if (v.fields.first.name != null) {
                // struct variant
                for (final field in v.fields) {
                  final fieldName = field.name ?? '_unnamed';
                  final fieldTypeExp = use(field.type);
                  final isOpt = _isOptional(field.type) ? '?' : '';
                  out.line('$fieldTypeExp$isOpt $fieldName,');
                }
              } else if (v.fields.length == 1) {
                // single unnamed field
                final fieldTypeExp = use(v.fields.first.type);
                final isOpt = _isOptional(v.fields.first.type) ? '?' : '';
                out.line('$fieldTypeExp$isOpt value,');
              } else {
                // multiple unnamed fields => treat as tuple
                final tupleExp =
                    _makeTuple(v.fields.map((f) => f.type).toList());
                out.line('$tupleExp value,');
              }
            }
          },
          end: '}) = $variantClassName;',
        );

        // Then define an inner class that implements $name
      }
      out.line('}');

      // Now define each variant as its own class
      for (final v in variant.variants) {
        final variantClassName = '${name}_${v.name}';
        out.line();
        out.blockComment(v.docs);
            ...
            if (v.fields.isNotEmpty) {
              if (v.fields.first.name != null) {
                for (final field in v.fields) {
                  ...
                  final fieldTypeExp = use(field.type);
                  final isOpt = _isOptional(field.type) ? '?' : '';
                  out.line('final $fieldTypeExp$isOpt $fieldName;');
                }
              } else {
                out.line('final dynamic value;');
              }
            }

            // Indicate the variant type name
            out.line();
            out.line('static const String kind = \'${v.name}\';');
          },
          end: '}',
        );
      }
  ...
  /// Returns `true` if the given type index is an `Option`;
  /*  bool _isOptional(int ti) {
    return sink.types[ti].kind == TypeKind.option;
  } */
