...
class PolkadartGenerator {
  ...
  Class createPolkadartRegistry() => Class((classBuilder) {
          ...
          (b) => b
            ...
            ..initializers.addAll(
              pallets
                  ...
                  .map(
                    (pallet) => Code.scope(
                      (a) => '${sanitize(pallet.name)} = ${a(pallet.queries(dirname))}(api)',
                    ),
                  ),
            ),
        ),
  ...
  Class createPolkadartTx() => Class((classBuilder) {
    ...
    classBuilder
      ...
      ..fields.addAll(
        pallets
            .where((pallet) => pallet.calls != null)
            .map(
              (pallet) => Field(
                (b) => b
                  ..name = sanitize(pallet.name)
        ...
        Constructor(
          (b) => b
            ...
            ..body = Block.of([
              ...
              refer(
                name,
              ).newInstanceNamed('_', [refer('provider'), refer('rpc')]).returned.statement,
            ]),
        ),
        Constructor(
          (b) => b
            ...
            ..body = Block.of([
              declareFinal(
                'provider',
              ).assign(refs.provider.newInstanceNamed('fromUri', [refer('url')])).statement,
      ...
      ..methods.addAll([
        Method(
          (b) => b
            ...
            ..body = refer('_provider').property('connect').call([]).awaited.returned.statement,
        ),
        Method(
          (b) => b
            ..name = 'disconnect'
            ..returns = refs.future()
            ..modifier = MethodModifier.async
            ..body = refer('_provider').property('disconnect').call([]).awaited.returned.statement,
        ),
      ]);
  });
}
