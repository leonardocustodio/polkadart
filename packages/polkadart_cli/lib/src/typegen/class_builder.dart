...
Method overrideEqualsMethod(Reference classType, List<generators.Field> fields) {
  ...
  for (final field in fields) {
    ...
    if (codec is generators.SequenceDescriptor || codec is generators.ArrayDescriptor) {
      body = body.and(
        refs.quiverListsEqual.call([
    ...
    body = body.and(
      refer(
        'other',
      ).property(fieldname).equalTo(refer(fieldname == 'other' ? 'this.other' : fieldname)),
    );
  }
...
Method overrideHashCodeMethod(List<generators.Field> fields) {
  return Method((b) {
    ...
    if (fields.isEmpty) {
      ...
      builder.body = refer(fields.first.sanitizedName).property('hashCode').code;
...
Class createCompositeClass(generators.CompositeBuilder compositeGenerator) => Class((classBuilder) {
  ...
  classBuilder
    ...
    ..methods.add(overrideEqualsMethod(classType, compositeGenerator.fields))
    ..methods.add(overrideHashCodeMethod(compositeGenerator.fields))
    ..fields.addAll(
      compositeGenerator.fields.map(
        (field) => Field(
          (b) => b
            ..name = field.sanitizedName
            ..type = field.codec.primitive(dirname)
            ..docs.addAll(field.rustTypeName != null ? sanitizeDocs([field.rustTypeName!]) : [])
                ...
                ..statements.add(
                  classType
                      .newInstance(
                        ...
                        {
                          for (var field in compositeGenerator.fields.where(
                            (field) => field.originalName != null,
                          ))
                    ...
                    (field) => refer('size')
                        .assign(
                          refer('size').operatorAdd(
                            field.codec.codecInstance(dirname).property('sizeHint').call([
                              refer('obj').property(field.sanitizedName),
                            ]),
                          ),
                        )
                        .statement,
          ...
          (b) => b
            ...
            ..body = compositeGenerator.fields.isEmpty
                ...
                : compositeGenerator.fields
                      .map(
                        (field) =>
                            field.codec.codecInstance(dirname).property('isSizeZero').call([]),
                      )
                      .reduce((a, b) => a.and(b))
                      .code,
        ),
...
Class createVariantBaseClass(generators.VariantBuilder generator, TypeReference jsonType) =>
    Class((classBuilder) {
        ...
        ..methods.add(
          Method(
            (b) => b
              ...
              ..returns = refs.uint8List
              ..body = Block.of([
                Code.scope((a) => 'final output = ${a(refs.byteOutput)}(codec.sizeHint(this));'),
...
Class createVariantValuesClass(generators.VariantBuilder variantGenerator) => Class((classBuilder) {
          ...
          (b) => b
            ...
            ..body = refer(variant.name)
                .call(
                  ...
                  {
                    for (final field in variant.fields.where((field) => field.originalName != null))
...
Class createVariantCodec(generators.VariantBuilder variantGenerator) => Class((classBuilder) {
  final dirname = p.dirname(variantGenerator.filePath);
      ...
      Method(
        (b) => b
          ...
          ..body = Block.of([
            declareFinal(
              'index',
            ).assign(generators.PrimitiveDescriptor.u8(1).decode(dirname)).statement,
            Code('switch (index) {'),
            Block.of(
              variantGenerator.variants.map(
                (variant) => Block.of([
    ...
    Method.returnsVoid(
      (b) => b
        ...
        ..body = Block(
          (b) => b
            ..statements.add(
              generators.PrimitiveDescriptor.u8(
                1,
              ).encode(dirname, literalNum(variant.index)).statement,
            )
            ..statements.addAll(
              variant.fields.map(
                (field) => field.codec
...
Enum createSimpleVariantEnum(generators.VariantBuilder variant) => Enum((enumBuilder) {
      ...
      variant.variants.map(
        (variant) => EnumValue(
          (b) => b
            ..name = generators.Field.toFieldName(variant.name)
            ..docs.addAll(sanitizeDocs(variant.docs))
            ..arguments.addAll([literalString(variant.originalName), literalNum(variant.index)]),
        ),
      ),
    )
    ..methods.add(
...
Class createSimpleVariantCodec(generators.VariantBuilder variant) => Class((classBuilder) {
    ...
    ..methods.add(
      Method.returnsVoid(
        (b) => b
          ...
          ..body = generators.PrimitiveDescriptor.u8(
            1,
          ).encode(dirname, refer('value').property('codecIndex')).statement,
      ),
    )
    ..methods.add(
      Method(
...
Class createTupleClass(int size) => Class((classBuilder) {
  classBuilder
    ..name = 'Tuple$size'
    ..constructors.add(
      Constructor(
...
Class createTupleCodec(int size) => Class((classBuilder) {
  final tupleType = TypeReference(
    (b) => b
      ..symbol = 'Tuple$size'
      ..types.addAll(List.generate(size, (index) => refer('T$index'))),
  );
          ...
          ..body = Block(
            (b) => b
              ..statements.addAll(
                List.generate(
                  size,
                  (index) => Code('codec$index.encodeTo(tuple.value$index, output);'),
                ),
              ),
          ),
      ),
        ...
        (b) => b
          ...
          ..body = Block(
            (b) => b
              ...
              ..statements.addAll(
                List.generate(size, (index) => Code('codec$index.decode(input),')),
              )
              ..statements.add(Code(');')),
          ),
      ),
          ...
          ..body = Block(
            (b) => b
              ...
              ..statements.addAll(
                List.generate(
                  size,
                  (index) => Code('size += codec$index.sizeHint(tuple.value$index);'),
                ),
              )
      ...
      Method(
        (b) => b
          ...
          ..body = size == 0
              ...
              : List.generate(
                  size,
                  (index) => refer('codec$index').property('isSizeZero').call([]),
                ).reduce((a, b) => a.and(b)).code,
      ),
    );
});
